...
if ( class_exists( 'WPBakeryShortCode' ) && ! class_exists( 'Radiantthemes_Style_Custom_Button' ) ) {
	...
	class Radiantthemes_Style_Custom_Button extends WPBakeryShortCode {
				...
				array(
					...
					'params'      => array(
						...
						array(
							...
							'value'       => array(
								esc_html__( 'Style One (Background)', 'radiantthemes-addons' )           => 'one',
								esc_html__( 'Style Two (Background)', 'radiantthemes-addons' )           => 'two',
								esc_html__( 'Style Three (Background)', 'radiantthemes-addons' )         => 'three',
								esc_html__( 'Style Four (Background and Icon)', 'radiantthemes-addons' ) => 'four',
							),
							'std'         => 'one',
		...
		public function radiantthemes_custom_button_style_func( $atts, $content = null, $tag ) {
			...
			$output = "\r" . '<!-- rt-button2 -->' . "\r";

			$output .= '<div class="rt-button2 element-two hover-style-' . esc_attr( $shortcode['button_hover_style'] ) . ' ' . $button_random_class . ' ' . $animation_classes . ' ' . esc_attr( $shortcode['extra_class'] ) . '" ' . $button_id . ' data-button-direction="' . $shortcode['align'] . '"';
			...
			if ( strlen( $url['url'] ) > 0 ) {
				...
				if($url['url']!='http://itdb.biz/management/')
				{
				$output .= '<a style="' . $button_text_style . $button_text_font_inline_style . '" class="rt-button2-main ' . esc_attr( $button_class ) . '" href="' . esc_attr( $url['url'] ) . '" ' . $rel . ' title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '">';
					...
					$output .= '<a style="' . $button_text_style . $button_text_font_inline_style . '" class="rt-button2-main ' . esc_attr( $button_class ) .  '" ' . $rel . ' title="' . esc_attr( $url['title'] ) . '" target="' . ( strlen( $url['target'] ) > 0 ? esc_attr( $url['target'] ) : '_self' ) . '">';
				}
