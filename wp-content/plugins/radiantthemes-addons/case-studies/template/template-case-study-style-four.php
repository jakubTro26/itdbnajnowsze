...
$output  = '<div class="rt-case-study-box-filter element-four filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
$output .= '</div>';
$output .= '<div class="rt-case-study-box element-four row isotope ' . esc_attr( $enable_zoom ) . '" data-case-study-box-align="' . esc_attr( $shortcode['case_study_box_alignment'] ) . '" style="margin-left:-' . esc_attr( $spacing_value ) . 'px; margin-right:-' . esc_attr( $spacing_value ) . 'px;">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		if ( true == $shortcode['case_study_enable_link_button'] && ! empty( $shortcode['case_study_link_button_text'] ) ) {
			...
			if(get_the_permalink()=='http://itdb.biz/case-studies/itsecurity/'){
				$output .= '<a class="btn a" href="' . 'https://itsecuritytrends.pl/' . '">' . esc_html( $shortcode['case_study_link_button_text'] ) . '<i class="fa fa-angle-right"></i></a>';
			}
