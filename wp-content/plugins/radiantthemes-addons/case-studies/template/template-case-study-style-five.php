<?php
/**
 * Template for Case Study Style Five
 *
 * @package Radiantthemes
 */

$output  = '<div class="rt-case-study-box-filter element-five filter-style-' . esc_attr( $shortcode['case_study_filter_style'] ) . ' text-' . esc_attr( $shortcode['case_study_filter_alignment'] ) . ' ' . esc_attr( $hidden_filter ) . '">';
...
$output .= '</div>';
$output .= '<div class="rt-case-study-box element-five row isotope ' . esc_attr( $enable_zoom ) . '" data-case-study-box-align="' . esc_attr( $shortcode['case_study_box_alignment'] ) . '" style="margin-left:-' . esc_attr( $spacing_value ) . 'px; margin-right:-' . esc_attr( $spacing_value ) . 'px;">';
...
if ( $my_query->have_posts() ) {
	while ( $my_query->have_posts() ) :
		...
		$output .= '</div>';
		$output .= '<div class="data matchHeight a">';
	    ...
	    $output .= '<a class="arrow-link" href="' . get_the_permalink() . '"><i class="fa fa-arrow-right"></i></a>';
		$output .= '</div>';
		$output .= '</div>';
		$output .= '</div>';
	endwhile;
